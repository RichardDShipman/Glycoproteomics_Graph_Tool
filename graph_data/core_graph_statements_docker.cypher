...
CREATE CONSTRAINT IF NOT EXISTS FOR (gt:Glycotransferase) REQUIRE gt.uniprotkb_canonical_ac IS UNIQUE;
CREATE CONSTRAINT IF NOT EXISTS FOR (gh:Glycohydrolase) REQUIRE gh.uniprotkb_canonical_ac IS UNIQUE;
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/mouse_protein_masterlist.csv' AS row
MERGE (n:Protein {
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_protein_masterlist.csv' AS row
MERGE (n:Protein {
    uniprotkb_canonical_ac: row.uniprotkb_canonical_ac,
    status: row.status,
    gene_name: row.gene_name,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_protein_info_refseq.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/mouse_protein_info_uniprotkb.csv' AS row
MATCH (n:Protein {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_protein_info_uniprotkb.csv' AS row
MATCH (n:Protein {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
SET n.uniprotkb_id = row.uniprotkb_id,
    n.uniprotkb_protein_mass = row.uniprotkb_protein_mass,
    n.uniprotkb_protein_length = row.uniprotkb_protein_length;
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_protein_xref_cazy.csv' AS row
MATCH (n:Protein {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
SET n.cazy = row.xref_id,
    n.cazy_label = row.xref_label;

// BRENDA

LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_protein_xref_brenda.csv' AS row
MATCH (n:Protein {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
SET n.brenda_id = row.xref_id,
    n.brenda_label = row.xref_label;

// GLYCAN NODES AND RELATIONSHIPS

// Create or update Glycan nodes with data from the glycan master list CSV file
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_masterlist.csv' AS row
MERGE (n:Glycan {
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_subsumption.csv' AS row
...
// Create or update relationships between Glycan nodes based on data from the subsumption CSV file
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_subsumption.csv' AS row
WITH row WHERE row.glytoucan_type = 'Topology' AND row.glytoucan_ac <> row.related_accession
MATCH (n1:Glycan {glytoucan_ac: row.glytoucan_ac})
MATCH (n2:Glycan {glytoucan_ac: row.related_accession})
MERGE (n1)-[r1:HAS_TOPOLOGY]->(n2);

// BASE COMPOSITION

LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_subsumption.csv' AS row
WITH row WHERE row.glytoucan_type = 'BaseComposition' AND row.glytoucan_ac <> row.related_accession
MATCH (n1:Glycan {glytoucan_ac: row.glytoucan_ac})
MATCH (n2:Glycan {glytoucan_ac: row.related_accession})
MERGE (n1)-[r1:HAS_BASE_COMPOSITION]->(n2);

// COMPOSITION

LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_subsumption.csv' AS row
WITH row WHERE row.glytoucan_type = 'Composition' AND row.glytoucan_ac <> row.related_accession
MATCH (n1:Glycan {glytoucan_ac: row.glytoucan_ac})
MATCH (n2:Glycan {glytoucan_ac: row.related_accession})
MERGE (n1)-[r1:HAS_COMPOSITION]->(n2);

// SACCHARIDE

LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_subsumption.csv' AS row
WITH row WHERE row.glytoucan_type = 'Saccharide' AND row.glytoucan_ac <> row.related_accession
MATCH (n1:Glycan {glytoucan_ac: row.glytoucan_ac})
MATCH (n2:Glycan {glytoucan_ac: row.related_accession})
MERGE (n1)-[r1:HAS_SACCHARIDE]->(n2);

// MONOSACCHRIDE COMPOSITION

LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_monosaccharide_composition.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_sequences_byonic.csv' AS row
MATCH (n:Glycan {glytoucan_ac: row.glytoucan_ac})
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_type_n_linked_byonic.csv' AS row
MATCH (n:Glycan {glytoucan_ac: row.glytoucan_ac})
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_dictionary.csv' AS row
MATCH (n:Glycan {glytoucan_ac: row.glytoucan_ac})
SET n.term = row.term,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_sequences_glycam_iupac.csv' AS row
MATCH (n:Glycan {glytoucan_ac: row.glytoucan_ac})
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_sequences_glycoct.csv' AS row
MATCH (n:Glycan {glytoucan_ac: row.glytoucan_ac})
SET n.term = row.term,
n.term_in_sentence = row.term_in_sentence;

// Update Glycan nodes with InChI sequence and InChI key data from the CSV file
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_sequences_inchi.csv' AS row
MATCH (n:Glycan {glytoucan_ac: row.glytoucan_ac})
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_sequences_iupac_extended.csv' AS row
MATCH (n:Glycan {glytoucan_ac: row.glytoucan_ac})
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_sequences_smiles_isomeric.csv' AS row
MATCH (n:Glycan {glytoucan_ac: row.glytoucan_ac})
SET n.sequence_smiles_isomeric = row.sequence_smiles_isomeric,
n.pubchem_id = row.pubchem_id;

LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_sequences_wurcs.csv' AS row
MATCH (n:Glycan {glytoucan_ac: row.glytoucan_ac})
SET n.sequence_wurcs = row.sequence_wurcs;

// Update Glycan nodes with GWB sequence data from the CSV file
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_sequences_gwb.csv' AS row
MATCH (n:Glycan {glytoucan_ac: row.glytoucan_ac})
SET n.sequence_gwb = row.sequence_gwb;

// Update Glycan nodes with name and domain information from the CSV file
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_names.csv' AS row
MATCH (n:Glycan {glytoucan_ac: row.glytoucan_ac})
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_motif.csv' AS row
MERGE (n:Motif {
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_classification.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/mouse_protein_genelocus.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_protein_genelocus.csv' AS row
MERGE (n:Gene { ensembl_gene_id: row.ensembl_gene_id })
SET n.gene_symbol = row.gene_symbol,
    n.chromosome_id = row.chromosome_id,
    n.start_pos = row.start_pos,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_species.csv' AS row
MERGE (n:Organism {
    taxid: row.tax_id,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_enzyme.csv' AS row
MERGE (n:Enzyme {
    uniprotkb_canonical_ac: row.uniprotkb_canonical_ac,
...
MATCH (n1:Monosaccharide {residue_id: row.residue_id + '-' + row.glytoucan_ac})
MATCH (n2:Glycan {glytoucan_ac: row.glytoucan_ac})
// Merge 'HAS_LINKAGE' relationship between Monosaccharide and Glycan nodes
MERGE (n1)<-[r1:HAS_LINKAGE]-(n2)
SET r1.glytoucan_ac = row.glytoucan_ac;

// GLYCOSYLATIONSITES NODES AND RELATAIONSHIPS

LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/mouse_proteoform_glycosylation_sites_uniprotkb.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_proteoform_glycosylation_sites_uniprotkb.csv' AS row
MERGE (n:GlycosylationSite { protein_glycosylation_site: row.uniprotkb_canonical_ac + '-' + row.glycosylation_site_uniprotkb })
ON CREATE SET
    n.uniprotkb_canonical_ac = row.uniprotkb_canonical_ac,
    n.glycosylation_site_uniprotkb = row.glycosylation_site_uniprotkb,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_proteoform_glycosylation_sites_gptwiki.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/mouse_protein_go_annotation.csv' AS row
MERGE (n:GeneOntology {go_term_id: row.go_term_id})
ON CREATE SET
    n.go_term_label = row.go_term_label,
    n.go_term_category = row.go_term_category,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/glycan_citations_glytoucan.csv' AS row
...
MERGE (n1)-[r1:REFERERENCED_IN]->(n2);

// PROTEIN CITATIONS NODES AND RELATIONSHIPS

LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_protein_citations_uniprotkb.csv' AS row
MERGE (n:Citation {title: row.title})
ON CREATE SET
    n.journal_name = row.journal_name,
    n.publication_date = row.publication_date,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/mouse_protein_citations_uniprotkb.csv' AS row
MERGE (n:Citation {title: row.title})
ON CREATE SET
    n.journal_name = row.journal_name,
    n.publication_date = row.publication_date,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_proteoform_citations_glycosylation_sites_unicarbkb.csv' AS row
MERGE (n:Citation {title: row.title})
ON CREATE SET
    n.journal_name = row.journal_name,
    n.publication_date = row.publication_date,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/mouse_proteoform_citations_glycosylation_sites_unicarbkb.csv' AS row
MERGE (n:Citation {title: row.title})
ON CREATE SET
    n.journal_name = row.journal_name,
    n.publication_date = row.publication_date,
...
// GLYCOTRANSFERASE AND GLYCOHYDROLASE NODES AND RELATIONS

LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_protein_glycosyltransferase.csv' AS row
MERGE (n:Glycotransferase {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
ON CREATE SET
    n.species = row.species,
    n.status = row.status,
    n.gene_symbol = row.gene_symbol,
    n.uniprotkb_protein_name = row.uniprotkb_protein_name,
    n.ec_number = row.ec_number,
    n.brenda_ec_number = row.brenda_ec_number,
    n.cazy_gt_family = row.cazy_gt_family,
    n.interpro_id = row.interpro_id,
    n.pfam_ac = row.pfam_ac,
    n.go_molecular_function = row.go_molecular_function
ON MATCH SET
    n.species = row.species,
    n.status = row.status,
    n.gene_symbol = row.gene_symbol,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_protein_glycohydrolase.csv' AS row
MERGE (n:Glycohydrolase {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
ON CREATE SET
    n.species = row.species,
    n.status = row.status,
    n.gene_symbol = row.gene_symbol,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_protein_glycosyltransferase.csv' AS row 
MATCH (n1:Protein {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
MATCH (n2:Glycotransferase {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
MERGE (n1)-[r1:IS_A]->(n2);

LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/human_protein_glycohydrolase.csv' AS row 
MATCH (n1:Protein {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
MATCH (n2:Glycohydrolase {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
MERGE (n1)-[r1:IS_A]->(n2);

LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/mouse_protein_glycosyltransferase.csv' AS row
MERGE (n:Glycotransferase {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
ON CREATE SET
    n.species = row.species,
    n.status = row.status,
...
LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/graph_data/mouse_protein_glycohydrolase.csv' AS row
MERGE (n:Glycohydrolase {uniprotkb_canonical_ac: row.uniprotkb_canonical_ac})
ON CREATE SET
    n.species = row.species,
    n.status = row.status,
